...
def hog_extraction(normB, normG, normR):
    ...
    fd_B, hog_image_B = hog(normB, orientations=8, pixels_per_cell=(8, 8), cells_per_block=(
        1, 1), visualize=True, channel_axis=-1, multichannel=False, feature_vector=True)
    fd_G, hog_image_G = hog(normG, orientations=8, pixels_per_cell=(8, 8), cells_per_block=(
        1, 1), visualize=True, channel_axis=-1, multichannel=False, feature_vector=True)
    fd_R, hog_image_R = hog(normR, orientations=8, pixels_per_cell=(8, 8), cells_per_block=(
        1, 1), visualize=True, channel_axis=-1, multichannel=False, feature_vector=True)
