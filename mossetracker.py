            ...
            if self.ResNet == False and self.color == False:
                ...
                if self.HOG == True:
                    hog_image_pre = cv2.resize(
                        i_img_norm, (w*4, h*4))
                    i_img_norm, hog_img = hog_extraction(hog_image_pre)
                    i_img_norm = i_img_norm.squeeze()
                    if fd is None:
                        fd = i_img_norm
                    else:
                        fd = np.concatenate([i_img_norm, fd])
            elif self.ResNet == True:
                i_img_norm = img_color_mode[0, :, :, i]
            else:
                i_img_norm = img_color_mode[:, :, i]
            if self.HOG == True:
                _, _, hog_channels = fd.shape
                for i in range(hog_channels):
                    i_F = np.fft.fft2(fd[:, :, i])
